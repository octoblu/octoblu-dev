...
tmux send-keys -t $SESSION:1.0 '(cd $SERVICES; ./run-service.sh oauth-provider)' C-m

tmux send-keys -t $SESSION:2.0 'cd ~/Projects/Octoblu/device-mailer' C-m
tmux send-keys -t $SESSION:2.0 '(cd $SERVICES; ./run-service.sh device-mailer)' C-m

tmux send-keys -t $SESSION:3.0 'cd ~/Projects/Octoblu/device-editor-octoblu' C-m
tmux send-keys -t $SESSION:3.0 '(cd $SERVICES; ./run-service.sh device-editor-octoblu)' C-m
